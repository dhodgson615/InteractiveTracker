"""Repository pattern for managing student data persistence."""
...
class StudentRepository:
    """Handles all student data persistence operations."""
    ...
    def load_all_students(_self) -> pandas.DataFrame:
        """Load all student data from CSV file."""
        try:
            return pandas.read_csv(_self._csv_path)
        except FileNotFoundError:
            ...
            df.to_csv(_self._csv_path, index=False)
            return df
    ...
    def update_student_lesson(self, student_name: str, note: str = "") -> Optional[Student]:
    ...
    def get_unique_first_letters(self) -> list[str]:
        ...
        df_valid = df.dropna(subset=["name"])
        first_letters = set([name[0].upper() for name in df_valid["name"] if name])
