...
def create_icon():
    ...
    if not PIL_AVAILABLE:
        ...
        print(
            "3. Or manually create an icon and place it in 'Interactive Tracker.app/Contents/Resources/icon.png'"
        )
    ...
    for i in range(3, 8):
        ...
        draw.line(
            [margin + 20, y, size - margin - 30, y], fill=line_color, width=3
        )
    ...
    # Pen body
    draw.rectangle(
        [pen_x, pen_y, pen_x + pen_width, pen_y + pen_length], fill=pen_color
    )
    ...
    draw.text(
        (text_x + 2, text_y + 2), text, fill=(0, 0, 0, 128), font=font
    )  # Shadow

    draw.text(
        (text_x, text_y), text, fill=(255, 255, 255, 255), font=font
    )  # Main text
    ...
    for s in sizes:
        ...
        resized.save(
            f"Interactive Tracker.app/Contents/Resources/icon_{s}.png", "PNG"
        )
